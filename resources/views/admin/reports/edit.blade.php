    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.reports.update", [$report->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('gtnn_number') ? 'is-invalid' : '' }}" name="gtnn_number_id" id="gtnn_number_id" required>
                    @foreach($gtnn_numbers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('gtnn_number_id') ? old('gtnn_number_id') : $report->gtnn_number->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('time') ? 'is-invalid' : '' }}" type="text" name="time" id="time" value="{{ old('time', $report->time) }}">
                @if($errors->has('time'))
                    <div class="invalid-feedback">
                        {{ $errors->first('time') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('report_status') ? 'is-invalid' : '' }}" name="report_status" id="report_status" required>
                    ...
                    @foreach(App\Models\Report::REPORT_STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('report_status', $report->report_status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('release') ? 'is-invalid' : '' }}" name="release" id="release" required>
                    ...
                    @foreach(App\Models\Report::RELEASE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('release', $report->release) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('admin_signature') ? 'is-invalid' : '' }}" type="text" name="admin_signature" id="admin_signature" value="{{ old('admin_signature', $report->admin_signature) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date_approved') ? 'is-invalid' : '' }}" type="text" name="date_approved" id="date_approved" value="{{ old('date_approved', $report->date_approved) }}">
