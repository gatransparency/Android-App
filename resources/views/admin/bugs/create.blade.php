    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.bugs.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('version') ? 'is-invalid' : '' }}" name="version_id" id="version_id" required>
                    @foreach($versions as $id => $entry)
                        <option value="{{ $id }}" {{ old('version_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('fixed') ? 'is-invalid' : '' }}" type="text" name="fixed" id="fixed" value="{{ old('fixed') }}">
                @if($errors->has('fixed'))
                    <div class="invalid-feedback">
                        {{ $errors->first('fixed') }}
                    </div>
