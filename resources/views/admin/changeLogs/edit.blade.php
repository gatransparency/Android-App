    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.change-logs.update", [$changeLog->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('portal_version') ? 'is-invalid' : '' }}" name="portal_version_id" id="portal_version_id" required>
                    @foreach($portal_versions as $id => $entry)
                        <option value="{{ $id }}" {{ (old('portal_version_id') ? old('portal_version_id') : $changeLog->portal_version->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('change') ? 'is-invalid' : '' }}" name="change" id="change" required>
                    ...
                    @foreach(App\Models\ChangeLog::CHANGE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('change', $changeLog->change) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('log') ? 'is-invalid' : '' }}" name="log" id="log" required>{{ old('log', $changeLog->log) }}</textarea>
                @if($errors->has('log'))
                    <div class="invalid-feedback">
                        {{ $errors->first('log') }}
                    </div>
