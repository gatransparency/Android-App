...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.internalInvestigation.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.internal-investigations.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="ia_date">{{ trans('cruds.internalInvestigation.fields.ia_date') }}</label>
                <input class="form-control date {{ $errors->has('ia_date') ? 'is-invalid' : '' }}" type="text" name="ia_date" id="ia_date" value="{{ old('ia_date') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('agency_office') ? 'is-invalid' : '' }}" name="agency_office_id" id="agency_office_id" required>
                    @foreach($agency_offices as $id => $entry)
                        <option value="{{ $id }}" {{ old('agency_office_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('investigator') ? 'is-invalid' : '' }}" type="text" name="investigator" id="investigator" value="{{ old('investigator', '') }}" required>
